...
2. 开启右上角的「开发者模式」。
3. 点击「加载已解压的扩展程序」。
4. 选择本目录：`/Users/xy/Downloads/谷歌插件/浏览器插件-视频转webp`，或解压 `dist/video-webm-webp-compressor-2.0.5.zip` 后选择解压目录。
...
然后回到插件里点击「开始转换」。插件会优先使用本地原生 FFmpeg；如果助手没有启动，才会回退到浏览器 `ffmpeg.wasm`。
